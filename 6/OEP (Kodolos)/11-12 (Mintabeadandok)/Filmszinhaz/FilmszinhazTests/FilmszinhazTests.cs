...
public class FilmszinhazTests
{
	...
	{
		Szemely pisti = new("Pisti", 3000);
		Ules ules = new('A', 1);
		Foglalas pistie = new Gyerek(pisti.nev, ules);

		Szemely.Foglal(szinhaz, "Maugli", reggel, Terem.KOZEPES, pistie);
		Assert.IsTrue(eloadas1.FoglaltHelyek.Any(x => x == pistie));

		Szemely.Lemond(szinhaz, "Maugli", reggel, Terem.KOZEPES, pistie);
	...
	{
		Szemely jozsi = new("Józsi", 2900);
		Foglalas jozsie = new Felnott(jozsi.nev, new Ules('A', 11));

		Assert.ThrowsException<NemLetezoUlesException>(() => Szemely.Foglal(szinhaz, "Pretty woman", reggel, Terem.NAGY, jozsie));
