...
public class InFile
{
	...
	public void SzinhazBeolvasasa(out Szinhaz szinhaz)
	{
		...
		try
		{
			...
			{
				Console.WriteLine("\nVásárló érkezett:");
				Szemely sz = new(tokenek[1].Replace('_', ' '), int.Parse(tokenek[2]));
				while (OlvassTovabb(ref hasznaltSor, "Tesz", ref sor, ref tokenek, szeparator))
				{
					Ido ido = IdotOlvas(tokenek[3], tokenek[4]);
					Ules ules = new(tokenek[7][0], int.Parse(tokenek[7].Split(',')[1]));
					...
					try
					{
						...
						{
							case "Foglal":
								Szemely.Foglal(szinhaz, tokenek[2].Replace('_', ' '), ido, terem, foglalas);
								Console.WriteLine("Sikeres foglalás.");
								break;
							case "Lemond":
								Szemely.Lemond(szinhaz, tokenek[2].Replace('_', ' '), ido, terem, foglalas);
