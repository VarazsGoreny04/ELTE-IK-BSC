...
public struct Real
{
	...
	private const uint MAX = 64u;

	public static readonly UInt128 MAXWHOLEINT = (UInt128)Math.Pow(2, MAX) - 1u;
	...
	public static readonly UInt128 MAXFRACTIONINT = Convert.ToUInt32(new string('9', (int)FRACTIONLENGTH));
	...
	private readonly unsafe bool sign;
	...
	public Real(string num)
	{
		...
		UInt128 wholeInt, fractionInt;

		try
		{
			wholeInt = UInt128.Parse(tokens[0][1..]);

			UInt128 holdUp = 1;
            for (int i = 0; i < FRACTIONLENGTH - tokens[1].Length; ++i)
				holdUp *= 10;

            fractionInt = UInt128.Parse(tokens[1]) * holdUp;
		}
		catch
		{
			throw new CalculationError();
	...
	private static (bool[] Bits, int Power) ToBits(UInt128 num)
	{
		int pow = 0;
	...
	private static UInt128 ToUInt(bool[] b)
	{
	...
	{
		UInt128 wholeInt = ToUInt(whole);

		UInt128 fractionInt = ToUInt(fraction);
