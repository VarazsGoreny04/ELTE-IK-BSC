...
public class Simulation {
    ...
    public static Integer preparingDetectiveCount = detectives.size();
    ...
    public static void main(String[] args) {
        ...
        detectives.forEach(detective -> {detectiveExecutor.submit( () -> {detective.interrogate(perpQueue, lock);} );});
        ...
        try{
            ...
            executionExecutor.awaitTermination(20000, TimeUnit.MILLISECONDS);
            detectiveExecutor.awaitTermination(20000, TimeUnit.MILLISECONDS);
        ...
        try{
            ...
            executionExecutor.awaitTermination(10, TimeUnit.MILLISECONDS);
            detectiveExecutor.awaitTermination(10, TimeUnit.MILLISECONDS);
        }catch (InterruptedException e){
            e.printStackTrace();
        }finally {
            executionExecutor.shutdownNow();
    ...
    private static void catchCrimeBosses() {
        ...
        synchronized(lock) {
            while(preparingDetectiveCount > 0) {
                try{
                    lock.wait();
                }catch (InterruptedException e) {
                    throw new RuntimeException(e);
