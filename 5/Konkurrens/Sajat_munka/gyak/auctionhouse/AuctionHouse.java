...
public class AuctionHouse {
    ...
    private static List<Thread> makeArtists() {
        // TODO create ARTIST_COUNT artists as threads, all of whom do the following,
        ...
        for (int i = 0; i < ARTIST_COUNT; ++i) {
            ...
            artists.add(new Thread(() -> {
                ...
                int localPrice = rnd.nextInt(100, 1000);
                while (true) {
                    synchronized (nfts) {
                        if (nftIdx != MAX_NFT_IDX && remainingNftPrice >= localPrice) {
                            try {
                                Thread.sleep(20);
                            } catch (InterruptedException e) {
                            }
                            NFT newNft = new NFT(index, localPrice);
                            nfts[nftIdx] = newNft;
                            remainingNftPrice -= localPrice;
                            ++nftIdx;
                        } else
                            break;
                    }
    ...
    // run an auction if 1. any artists are still working 2. run 100 auctions after
    // all artists are finished
    ...
    // only for Task 3: if an offer is made and it has a better price than all
    // previous ones, this is the currently winning offer
    // once the auction is done, add the commission to totalCommission
    // the commission is 10% of the price of the NFT (including the sum in the
    // highest offer, if there was any)
    // only for Task 3: if there was an offer, increase soldItemCount and remember
    // that the collector owns an NFT
    ...
    private static List<Thread> makeCollectors(Thread auctioneer) {
        // TODO create collectors now, the collectors' names are simply Collector1,
        // Collector2, ...

        // work until the auctioneer is done (it is not isAlive() anymore)
        // sleep for COLLECTOR_MIN_SLEEP..COLLECTOR_MAX_SLEEP milliseconds randomly
        // between each step
        // if there is no auction available, just increase noAuctionAvailableCount
        // if there is an ongoing auction, and you haven't bid on it already, make an
        // offer
    ...
    private static void runChecks() {
        if (Thread.activeCount() == 1) {
            ...
            System.out.printf("%s %d threads are active, there should be only one%n", isOK(Thread.activeCount() == 1),
                    Thread.activeCount());
        }
        ...
        System.out.printf("%s Money is not lost: %d + %d = %d%n", isOK(soldPrice + remainingNftPrice == INIT_ASSETS),
                soldPrice, remainingNftPrice, INIT_ASSETS);

        System.out.printf("%s [Only Task 2] Total commission not zero: %d > 0%n", isOK(totalCommission > 0),
                totalCommission, INIT_ASSETS);

        System.out.printf("%s [Only Task 3] Sold item count not zero: %d > 0%n", isOK(soldItemCount > 0), soldItemCount,
                INIT_ASSETS);
        System.out.printf("%s [Only Task 3] Some collectors have become owners of NFTs: %d > 0%n",
                isOK(owners.size() > 0), owners.size(), INIT_ASSETS);
        System.out.printf("%s [Only Task 3] Sometimes, collectors found no auction: %d > 0%n",
                isOK(noAuctionAvailableCount > 0), noAuctionAvailableCount, INIT_ASSETS);

        System.out.printf("%s Altogether %d condition%s failed%n", isOkTxt(failCount == 0), failCount,
                failCount == 1 ? "" : "s");
