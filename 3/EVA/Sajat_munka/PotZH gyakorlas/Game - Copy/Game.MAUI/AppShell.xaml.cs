...
public partial class AppShell : Shell
{
	...
	public AppShell(IStore GameStore, GameGameModel GameGameModel, GameViewModel GameViewModel)
	{
		InitializeComponent();
		...
		_timer.Tick += (_, _) => _GameViewModel.StepGame();

		_GameViewModel.NewGame += NewGame;
		_GameViewModel.Pause += Pause;
		_GameViewModel.LoadGame += LoadGame;
		_GameViewModel.SaveGame += SaveGame;
	...
	private void Pause(object? sender, EventArgs e)
	{
		if (_timer.IsRunning)
		{
			_GameViewModel.Paused = true;
			StopTimer();
		}
		else
		{
			_GameViewModel.Paused = false;
			StartTimer();
		}
	}
